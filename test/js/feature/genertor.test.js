const Generator = {
  async generateImages() {
    ...
    let basePrompt = `A beautiful product design for ${AppState.userProductName}, ${selectedTemplates.join(', ')} style${extra}, high quality, detailed, professional`;

    // If TemplateManager with learning is available, enrich the prompt with the learned style bias
    if (window.TemplateManager && typeof window.TemplateManager.enrichPrompt === 'function') {
      try {
        basePrompt = window.TemplateManager.enrichPrompt(basePrompt);
      } catch (e) {
        console.warn('Failed to enrich prompt with learned style bias:', e);
      }
    }
    ...
    try {
      const images = await this.generateImagePair(basePrompt);

      if (images.length === 2) {
        // Ensure renderer has global images for feedback handling
