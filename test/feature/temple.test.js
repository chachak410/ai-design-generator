...
${values.map((value) => `<option value="${value}">${value}</option>`).join('')}
  ...
  setupImageUpload() {
    // Keep existing behavior for legacy single upload if present elsewhere,
    // but now wire up two inputs and auto-combine when both uploaded.
    this.referenceFiles = { 1: null, 2: null };
    ...
    const handleFile = async (index, file) => {
      ...
      if (file.size > 10 * 1024 * 1024) { // allow up to 10MB each
        UI.showMessage('template-status', 'Image size exceeds 10MB limit.', 'error');
        return;
      }
      this.referenceFiles[index] = file;

      // if the other slot is already filled, trigger combine
      this.combineReferenceImagesIfReady();
    };
    ...
    // Backwards compatibility: keep existing single 'reference-image' input if present
    const legacy = document.getElementById('reference-image');
    if (legacy) {
      legacy.addEventListener('change', async (e) => {
        const file = e.target.files[0];
        if (file) {
          // If the user only uses legacy single upload, keep previous behavior:
          try {
  ...
  async combineReferenceImagesIfReady() {
    // Called after either input changes. If both files present, POST to /api/combine-description
    const file1 = this.referenceFiles?.[1];
    const file2 = this.referenceFiles?.[2];
    const loadingEl = document.getElementById('reference-images-loading');
    const box = document.getElementById('combined-description-box');
    const content = document.getElementById('combined-description-content');

    if (!file1 || !file2) {
      // hide box if incomplete
    ...
    try {
      ...
      fd.append('image1', file1, file1.name);
      fd.append('image2', file2, file2.name);

      // Send to backend
      const resp = await fetch('/api/combine-description', {
        method: 'POST',
        body: fd,
      });
      ...
      // backend should return JSON like { description: "..." }
      let data;
      ...
      this.renderCombinedDescription(desc);
    } catch (err) {
      console.error('[TemplateManager] combine-description error', err);
      UI.showMessage('template-status', 'Failed to combine images: ' + (err.message || err), 'error');
    } finally {
      if (loadingEl) { loadingEl.style.display = 'none'; }
    }
  },

  renderCombinedDescription(text) {
    const box = document.getElementById('combined-description-box');
    const content = document.getElementById('combined-description-content');
