...
function showMainApp() {
  ...
  if (authContainer && mainApp) {
    ...
    checkProductWarning();
    const needsSetup = currentUser && (!currentUser.displayName || !userProductName || userProductName.trim().length < 2);
    showPage(needsSetup ? 'setup-page' : 'template-page');
  } else {
...
// === IMAGE GENERATION: POLLINATIONS PRIMARY + FIXES ===
...
// PRIMARY: Pollinations AI (FIXED 502/524)
async function generateWithPollinations(prompt, seed, width = 768, height = 1024, retries = 6) {
  try {
...
// MAIN GENERATION: Pollinations First
async function generateImages() {
  ...
  showMessage('template-status', `Generating with Pollinations AI...`, 'info');
  ...
  while (images.length < 2 && attempts < maxAttempts && generationCount < 20) {
    ...
    try {
      ...
      if (image) {
        ...
        console.warn(`No image from ${llm}, trying next`);
      }
    } catch (err) {
  ...
  try {
    if (images.length === 2) {
      ...
      showMessage('template-status', `Got ${images.length}/2 images. Try again in 30s.`, 'error');
    }
  } catch (err) {
    showMessage('template-status', 'Error saving: ' + err.message, 'error');
  } finally {
    if (generateBtn) generateBtn.disabled = false;
  }
}

// === RENDER IMAGES + 4 BUTTONS (MATCHING index.html) ===
function renderImages(items) {
  ...
  // 插入按钮（带 data-i18n-button）
  fb.innerHTML = `
    <button id="btn-left-better"  class="btn-left-better"  data-i18n-button="btn-left-better"  onclick="downloadImage('${items[0].url}', 0)"></button>
    <button id="btn-right-better" class="btn-right-better" data-i18n-button="btn-right-better" onclick="downloadImage('${items[1].url}', 1)"></button>
    <button id="btn-tie"          class="btn-tie"          data-i18n-button="btn-tie"          onclick="downloadBoth()"></button>
    <button id="btn-both-bad"     class="btn-both-bad"     data-i18n-button="btn-both-bad"     onclick="generationCount++; generateImages()"></button>
  `;

  // 关键：使用 nextTick 确保 DOM 已更新
  requestAnimationFrame(() => {
    window.i18n.renderAll();           // 翻译按钮
    container.classList.remove('hidden'); // 显示容器
...
function downloadBoth() {
  const imgs = document.querySelectorAll('#image-pair img');
  imgs.forEach((img, i) => {
    const a = document.createElement('a');
    a.href = img.src;
  ...
  showMessage('template-status', 'Both images downloaded!', 'success');
}
